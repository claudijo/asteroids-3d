...
import { subdivide } from '../libs/mesh';
...
export const addAsteroids = (getState, dispatch, { minId, maxId, world }) => {
  ...
  dispatch(polyhedronComponent.add(2, {
    faces: subdivide(icosahedron, 0).map(face => face.map(vector => multiply(4, vector))),
    color: [255, 0, 0]
  }))
  ...
  dispatch(polyhedronComponent.add(3, {
    faces: subdivide(octahedron, 1).map(face => face.map(vector => multiply(6, vector))),
    color: [255, 0, 0]
  }))
