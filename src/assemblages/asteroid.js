...
import { length, multiply, toCartesian } from '../libs/vector';
...
const getFaces = cohort => {
  switch (cohort) {
    case 0:
      return distort(subdivide(icosahedron, 1)).map(face => face.map(vector => multiply(8, vector)));
    case 1:
      return distort(subdivide(octahedron, 1)).map(face => face.map(vector => multiply(6, vector)));
    case 2:
      return distort(subdivide(icosahedron, 0)).map(face => face.map(vector => multiply(4, vector)));
    default:
...
export const addAsteroid = (getState, dispatch, { cohort = 0, xPos, yPos, zPos = 0 }) => {
  ...
  const velocity = toCartesian([speed, random(0, 2 * Math.PI)])
